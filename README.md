...
- 默认会关闭 `cloakbrowser` 的版本更新提示与后台更新检查
- 同一 Node 进程里首次准备浏览器二进制时会自动串行化，避免并发 `launch()` 时重复下载
- 已兼容 `cloakbrowser@0.3.31` 的 ESM `exports` 限制，避免出现 `ERR_PACKAGE_PATH_NOT_EXPORTED`
...
- 默认关闭 `cloakbrowser` 的更新提示，并在同进程内只触发一次首次二进制准备
- 已兼容 `cloakbrowser@0.3.31` 的导出限制，避免首次准备二进制时报 `ERR_PACKAGE_PATH_NOT_EXPORTED`
